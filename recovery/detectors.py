            ...
            for pattern in self.BLOCK_TITLES:
                if pattern in title:
                    return DetectionResult(
                        ...
                        else BlockType.SOFT_RATE_LIMIT,
                        confidence=0.9,
                ...
                for pattern in self.BLOCK_PATTERNS[platform]:
                    if pattern in content:
                        return DetectionResult(
                            is_blocked=True,
                            block_type=BlockType.CAPTCHA
                            if "captcha" in pattern or "robot" in pattern
                            else BlockType.SOFT_RATE_LIMIT,
                            confidence=0.85,
...
class DetectorRegistry:
    ...
    def get_consensus(
        self, results: List[DetectionResult], threshold: float = 0.5
    ) -> DetectionResult:
        ...
        blocked_results = [
            r for r in results if r.is_blocked and r.confidence >= threshold
        ]
