...
async def _cmd_scrape(args: argparse.Namespace) -> int:
    ...
    print(
        f"[agentic-stealth-browser] Scraping: {url} (extract={extract}, platform={platform})"
    )
    ...
    try:
        async with AgentBrowser(
            ...
            # launch happens exactly once inside __aenter__ (preset/region forwarded via ctor)
            ...
            health = await browser.get_health_status()
            print(
                f"\n[agentic-stealth-browser] Scrape complete. Block rate: {health.get('block_rate_pct', 0)}%",
                file=sys.stderr,
            )
